...
class server:
    ...
    async def __cl_method_handler__(self, client: type, message: dict):
        ...
        if self.validate({"cmd": str}, message) == self.supporter.invalid:
            ...
            try:
                async for tmp_msg in client:
                    ...
                    if message:
                        ...
                        elif result == self.supporter.invalid:
                            if client.protocol == self.supporter.proto_cloudlink:
                                listener = self.supporter.detect_listener(message)
                        ...
                        elif result == self.supporter.missing_key:
                            if client.protocol == self.supporter.proto_cloudlink:
                                listener = self.supporter.detect_listener(message)
                                await self.send_code(client, "Syntax", listener=listener)
                        ...
                        elif result == self.supporter.unknown_method:
                                if client.protocol == self.supporter.proto_cloudlink:
                                    listener = self.supporter.detect_listener(message)
                        ...
                        elif result ==self.supporter.unknown_protocol:
...
class clients:
    ...
    def delete(self, client: dict):
        if self.exists(client):
            ...
            elif self.get(client).protocol == self.__proto_scratch_cloud__:
